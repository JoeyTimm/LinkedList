...
public:
	...
	/// with a default constructor, a destructor, an assignment operator, and a copy constructor can be used.
	typedef int value_type;

	/// @brief Sequence::size_type is the data type of any variable the number of items in a sequence.
	typedef unsigned int size_type;

	// CONSTRUCTORS for the sequence class:

	/// @brief Creates an empty sequence (numElts == 0) or a sequence of numElts items indexed from 0 ... (numElts - 1). 
	...
	/// @brief Destroys all items in the sequence and release the memory associated with the sequence
	~Sequence();

	/// @brief The current sequence is released and replaced by a (deep) copy of sequence (seq). A reference to the copied sequence is returned.
	...
	/// @brief The position satisfies ( position >= 0 && position <= last_index( ) ). 
	// The value of item is inserted at position and the size of sequence is increased by one.
	/// throws an exception	if the position is outside the bounds of the sequence
	...
	/// @brief All items in the sequence deleted and the memory associated with the sequence is released.
	void clear();

	/// @brief The position satisfies ( position >= 0 && position + count - 1 <= last_index( ) )
	/// throws an exception	if the sequence when called with invalid position and/or count
	/// The positions / items in the sequence at ( position ... (position + count - 1) ) are deleted and their memory is released
...
private:
	...
	public:
		...
		{}
		/// @brief parameterized constructor, next and prev are set to nullptr and the node's element is set to the given value
		/// @param item value the node's element will be set to
		SequenceNode(value_type item): next(nullptr), prev(nullptr), elt(item)
		{}
		/// @brief node destructor. Think about what it would mean and whether anything should be done here
		~SequenceNode() {};

	};

	// MEMBER DATA. These are the data items that each sequence object will contain. For a doubly-linked list, each sequence will have a head and tail pointer, and numElts
	SequenceNode* head;
	SequenceNode* tail;
